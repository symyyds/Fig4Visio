...
```text
原始图片 -> scene.json -> 结构校验 -> 复杂度预检 -> 模块审查 -> Visio COM 渲染 -> vsdx/svg/png -> 视觉对比 -> 迭代修正
```
...
如果你先改的是这个开源仓库，想把核心 skill 同步回 Codex 目录，可以运行：

```powershell
python .\sync_to_skill.py
```

默认会同步到 `%USERPROFILE%\.codex\skills\visiomaster`。同步后再去那个 skill 目录里测试渲染效果。
...
python .\scripts\font_inventory.py --check "Times New Roman" --check "Cambria Math" --check "Calibri"
python .\scripts\scene_complexity.py .\templates\examples\audit_region.scene.json --output .\exports\audit_region.complexity.md
...
对于很宽、很密、文字很多的论文图，建议先跑 `scene_complexity.py`。它会检查区域数量是否不足、节点是否没有归属区域、某个区域是否过密、同类组件字号是否漂移、文字是否可能装不下，以及节点是否疑似重叠。这个步骤的目的不是美化，而是在进 Visio 渲染前先把“大图越画越乱”的结构问题暴露出来。
